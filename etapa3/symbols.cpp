...
void SymbolTable::printTable() {
  ...
  std::cout << std::left << std::setw(20) << "LEXEMA" << std::setw(12) << "TIPO"
            << std::setw(8) << "CÓDIGO" << std::endl;
  std::cout << "----------------------------------------" << std::endl;
  ...
  if (table.empty()) {
    ...
    for (const auto &pair : table) {
      std::cout << std::left << std::setw(20) << pair.first << std::setw(12)
                << getTypeName(pair.second->type) << std::setw(8)
